        ...
        if ($message != null) {
            ...
            switch ($message->commande) {
                ...
                case "creer_salle":
                    ...
                    $salle =$message->utilisateur . date("G") . date("i");
    
                    // Ajoute le joueur à la salle
                    $clients[$message->utilisateur] = $clients["temp"];
                    unset($clients["temp"]);
                    $salles[$salle] = ["socket_hote" => $client, "mode" => $message->mode, "difficulte" => $message->difficulte, "hote" => $message->utilisateur, "hote_pret" => "0"];
    
                    // Renvoie le numéro de la salle au client
                    socket_write($client, mask(json_encode(["commande" => "numero_salle", "numero" => $salle])));
